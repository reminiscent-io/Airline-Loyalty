...
export function AtmosTierCard({ tier, highlighted = false }: AtmosTierCardProps) {
  ...
  const tierColors = {
    ...
    titanium: "bg-gradient-to-r from-atmos-green to-atmos-green/80 text-white"
  };
  ...
  return (
    <Card 
      className={cn(
        ...
        highlighted && "border-2 border-atmos-teal shadow-lg"
      )}
      data-testid={`card-tier-${tier}`}
    >
      {highlighted && (
        <div className="absolute top-0 right-0">
          <Badge className="rounded-none rounded-bl-lg bg-atmos-teal text-white border-0">
            Most Popular
          </Badge>
        </div>
      )}
      ...
      <CardHeader className="pb-4">
        ...
        <CardTitle className="text-2xl text-atmos-teal" data-testid={`text-tier-name-${tier}`}>
          {tier === "member" ? (
            "Base Tier"
          ) : tier === "silver" ? (
            "+25% Point Bonus"
      ...
      <CardContent>
        <div className="text-sm text-muted-foreground mb-4">
          <strong className="text-atmos-teal">
        ...
        <ul className="space-y-2">
          {config.benefits.map((benefit, index) => (
            ...
            >
              <Check className="w-4 h-4 text-atmos-green flex-shrink-0 mt-0.5" />
