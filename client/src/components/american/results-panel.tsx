...
import { type AmericanCalculationResults, type AmericanTierStatus, AMERICAN_TIER_CONFIGS } from "@shared/american-schema";
import { Separator } from "@/components/ui/separator";

interface AmericanResultsPanelProps {
  readonly results: AmericanCalculationResults | null;
}

const tierBadgeColors: Record<string, string> = {
  "executive-platinum": "bg-gray-800 text-white",
  "platinum-pro": "bg-purple-700 text-white",
  "platinum": "bg-gray-600 text-white",
  "gold": "bg-yellow-600 text-white",
};

const DEFAULT_BADGE_COLOR = "bg-american-blue text-white";

function getBadgeColor(tier: string): string {
  return tierBadgeColors[tier] ?? DEFAULT_BADGE_COLOR;
}

function getNextYearStatus(loyaltyPoints: number): AmericanTierStatus {
...
function TierMarkerLabel({ points, threshold, label }: Readonly<{ points: number; threshold: number; label: string }>) {
  const achieved = points >= threshold;
  return (
    <span className={`text-xs font-medium flex items-center gap-1 ${achieved ? 'text-green-600 font-semibold' : 'text-muted-foreground'}`}>
...
function PointsBreakdown({ results }: Readonly<{ results: AmericanCalculationResults }>) {
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        {(results.creditCardMiles > 0 || results.cardLoyaltyPoints > 0) && (
          <div className="flex items-center justify-between text-sm p-3 bg-muted/50 rounded-lg">
            <div className="flex items-center gap-2">
              <CreditCard className="w-4 h-4 text-american-red" />
        ...
        {results.partnerMiles > 0 && (
          <div className="flex items-center justify-between text-sm p-3 bg-muted/50 rounded-lg">
            <div className="flex items-center gap-2">
              <Building2 className="w-4 h-4 text-american-navy" />
...
export function AmericanResultsPanel({ results }: Readonly<AmericanResultsPanelProps>) {
  if (!results) {
    return (
  ...
  const totalSpent = results.returnOnSpend > 0
    ? (results.milesValue / results.returnOnSpend) * 100
    : 0;

  return (
    <Card className="border-2 border-american-blue/20" data-testid="card-results">
      <CardHeader>
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <CardTitle className="text-american-navy flex items-center gap-2">
            <Award className="w-5 h-5 text-american-blue" />
            Your Results
          </CardTitle>
          <Badge className="bg-american-blue text-white">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-5 bg-gradient-to-br from-american-red to-american-red/70 rounded-xl text-white shadow-lg">
          ...
          <div className="p-4 bg-gradient-to-br from-american-blue to-american-navy rounded-xl text-white">
          ...
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <div className="flex gap-4 flex-wrap">
              <div className="flex flex-col gap-1">
                ...
                <Badge className={getBadgeColor(results.currentTier)}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <Badge className={getBadgeColor(nextYearStatus)}>
                  <Award className="w-3 h-3 mr-1" />
          ...
          <div>
            ...
            <div className="relative">
              ...
              <div className="absolute top-0 left-0 w-full h-3 pointer-events-none">
                <div className="absolute top-0 h-full w-0.5 bg-background" style={{ left: '20%' }} />
                <div className="absolute top-0 h-full w-0.5 bg-background" style={{ left: '37.5%' }} />
                <div className="absolute top-0 h-full w-0.5 bg-background" style={{ left: '62.5%' }} />
              </div>
            </div>

            <div className="flex justify-between mt-1">
              <span className="text-xs text-muted-foreground">0</span>
              <TierMarkerLabel points={results.totalLoyaltyPoints} threshold={40000} label="40K (Gold)" />
              <TierMarkerLabel points={results.totalLoyaltyPoints} threshold={75000} label="75K (Platinum)" />
              <TierMarkerLabel points={results.totalLoyaltyPoints} threshold={125000} label="125K (Pro)" />
              <TierMarkerLabel points={results.totalLoyaltyPoints} threshold={200000} label="200K (Exec)" />
            </div>
        ...
        <PointsBreakdown results={results} />

        <Separator />

        {/* Financial Analysis */}
        <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
          <h4 className="font-semibold text-sm text-american-navy mb-3">Value Analysis</h4>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold flex items-center gap-1">
                <DollarSign className="w-3 h-3" />
                <span data-testid="text-total-spent">{totalSpent.toFixed(2)}</span>
              </span>
            </div>
            <div className="flex items-center justify-between">
